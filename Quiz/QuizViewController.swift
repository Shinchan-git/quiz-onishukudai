...
class QuizViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        quizArray.append(["iPhoneアプリ制作に使われるプログラミング言語は？", "Swift", "Soft", "Sweet", 1])
        //------------------------ここから上にクイズを書く------------------------//

        quizCount = quizArray.count
        quizArray.shuffle()
        choiceQuiz()
        styleButtons()
    }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if (segue.identifier == "toResultView") {
            ...
            resultView.quizCount = self.quizCount
    ...
    //Style
    func styleButtons() {
        choiceButtons1.layer.cornerRadius = 6
        choiceButtons2.layer.cornerRadius = 6
        choiceButtons3.layer.cornerRadius = 6
    }
}
